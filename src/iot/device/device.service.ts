...
export class DevicesService implements Observer {
    ...
    async registerDevice(type: string, status: string, data?: any): Promise<Device> {
    ...
    async createCommand(deviceId: string, createCommandDto: CreateCommandDto): Promise<Command> {
    ...
    async updateCommand(deviceId: string, commandId: string, updateCommandDto: CreateCommandDto): Promise<Command> {
        const device = await this.getDeviceById(deviceId);
        if (!device) {
